...
function getTime(date) {
  ...
  const result = timeString.split(' ')[0];
  return result;
}
...
function getNextFriday(date) {
  ...
  resultDate.setDate(resultDate.getDate() + daysUntilNextFriday);
...
function formatDate(date) {
  ...
  const formattedDate = `${month}/${day}/${year}, ${hours}:${minutes < 10 ? `0${minutes}` : minutes}:${seconds < 10 ? `0${seconds}` : seconds} ${amOrPm}`;
...
function getWeekNumberByDate(date) {
  ...
  weekStart.setDate(yearStart.getDate() + 4 - (yearStart.getDay() || 7));

  let weekNumber = Math.ceil(((data - weekStart) / 86400000 + 1) / 7);

  if (weekNumber === 0) {
    weekNumber = getWeekNumberByDate(new Date(data.getFullYear() - 1, 11, 31));
  } else if (
    weekNumber === 53 &&
    getWeekNumberByDate(new Date(data.getFullYear(), 11, 31)) === 1
  ) {
    weekNumber = 1;
  }
...
function getNextFridayThe13th(date) {
  ...
  while (true) {
    const potentialFridayThe13th = new Date(year, month, 13);

    if (
      potentialFridayThe13th.getDay() === 5 &&
      potentialFridayThe13th.getMonth() === month
    ) {
      return potentialFridayThe13th;
    }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    if (cycleDay <= countWorkDays) {
      schedule.push(formatData(currentDate));
    }
